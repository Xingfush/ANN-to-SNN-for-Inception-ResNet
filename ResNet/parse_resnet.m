% parse_resnet: function description
function snn = parse_resnet(net,link,percentile)
% 相对于 dagnn2snn 只需要注意拓扑结构的变化，加入 sum layer 就行
...
for l=1:numel(net.layers)
	...
	elseif isa(net.layers(l).block,'dagnn.Sum')
		...
		% 只有在 sum 的时候才动用 relu.scale or concat.scale
		for i=(net.layers(l).inputIndexes-1)
			if i~=l-1
				snn{i}.scale=max_activations(link(i,1))/max_activations(link(i,2));
			end
