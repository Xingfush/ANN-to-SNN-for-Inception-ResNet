function [performance,stats] = resconv2snn(snn,imdb,opts)
...
% -------------------------------------------------------------------------
for l=1:numel(snn)
	if strcmp(snn{l}.type,'conv') || strcmp(snn{l}.type,'sum')
...
for t=0:opts.dt:opts.duration
	...
	for l=2:numel(snn)
		...
		% sum 作为 conv 来处理，直接对上层的 spikes 处理，跳过 conv 层的 mem，这样和 short normalization 更契合
		% sum 上层 conv 失去作用，不必进行处理
		elseif strcmp(snn{l}.type,'sum')
			% prev(1) 必是 shortcut 上的 relu or conv
			if strcmp(snn{snn{l}.prev(1)}.type,'relu')
				i=snn{snn{l}.prev(2)}.prev;
				z=snn{snn{l}.prev(1)}.spikes*snn{snn{l}.prev(1)}.scale + ...
				  		vl_nnconv(snn{i}.spikes,snn{snn{l}.prev(2)}.weights{1},snn{snn{l}.prev(2)}.weights{2},...
								'stride',snn{snn{l}.prev(2)}.stride,'pad',snn{snn{l}.prev(2)}.pad);
			else
				i=snn{snn{l}.prev(2)}.prev;
				j=snn{snn{l}.prev(1)}.prev;
				z=vl_nnconv(snn{j}.spikes,snn{snn{l}.prev(1)}.weights{1},snn{snn{l}.prev(1)}.weights{2},...
                        'stride',snn{snn{l}.prev(1)}.stride,'pad',snn{snn{l}.prev(1)}.pad) +...
				  		vl_nnconv(snn{i}.spikes,snn{snn{l}.prev(2)}.weights{1},snn{snn{l}.prev(2)}.weights{2},...
								'stride',snn{snn{l}.prev(2)}.stride,'pad',snn{snn{l}.prev(2)}.pad);
            end
			snn{l}.mem=snn{l}.mem+z;
